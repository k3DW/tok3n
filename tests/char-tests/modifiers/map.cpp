...
constexpr auto map_modifier_fragment =
    []<detail::parser P>(P) {
        using R = map_parser<P, integral_constant<sink_func>>;
        EXPECT_THAT(the_parser<P> | is_modifiable_by<map<sink_func>>.TEMPLATE_IF_GCC12 with_result<R>);
    };

TEST("map modifier", "modify anything")
{
    EXPECT_THAT(all_samples.satisfy(map_modifier_fragment));
}
