...
template <class P>
concept Parser =
	requires { typename std::integral_constant<ParserFamily, P::type>; } &&
	static_cast<int>(P::type) > static_cast<int>(ParserFamily::None) &&
	static_cast<int>(P::type) < static_cast<int>(ParserFamily::END) &&
	(std::is_empty_v<P>) &&
