...
TEST("cpo index", "Unchecked")
{
    ...
    static constexpr auto packet = [](auto& state) {
        Unchecked u;
        EXPECT_STATEFUL(state, detail::index(u, 0) == 1);
        EXPECT_STATEFUL(state, detail::index(u, 1) == 2);
        EXPECT_STATEFUL(state, detail::index(u, 2) == 3);
        EXPECT_STATEFUL(state, detail::index(std::as_const(u), 0) == 2);
        EXPECT_STATEFUL(state, detail::index(std::as_const(u), 1) == 3);
        EXPECT_STATEFUL(state, detail::index(std::as_const(u), 2) == 4);
        EXPECT_STATEFUL(state, detail::index(Unchecked{}, 0) == 3);
        EXPECT_STATEFUL(state, detail::index(Unchecked{}, 1) == 4);
        EXPECT_STATEFUL(state, detail::index(Unchecked{}, 2) == 5);
        EXPECT_STATEFUL(state, detail::index(static_cast<const Unchecked&&>(u), 0) == 4);
        EXPECT_STATEFUL(state, detail::index(static_cast<const Unchecked&&>(u), 1) == 5);
        EXPECT_STATEFUL(state, detail::index(static_cast<const Unchecked&&>(u), 2) == 6);
    };
    EXPECT_THAT(packet);
...
TEST("cpo index", "Checked")
{
    ...
    static constexpr auto packet = [](auto& state) {
        Checked c;
        EXPECT_STATEFUL(state, detail::index(c, 0) == 5);
        EXPECT_STATEFUL(state, detail::index(c, 1) == 6);
        EXPECT_STATEFUL(state, detail::index(c, 2) == 7);
        EXPECT_STATEFUL(state, detail::index(std::as_const(c), 0) == 6);
        EXPECT_STATEFUL(state, detail::index(std::as_const(c), 1) == 7);
        EXPECT_STATEFUL(state, detail::index(std::as_const(c), 2) == 8);
        EXPECT_STATEFUL(state, detail::index(Checked{}, 0) == 7);
        EXPECT_STATEFUL(state, detail::index(Checked{}, 1) == 8);
        EXPECT_STATEFUL(state, detail::index(Checked{}, 2) == 9);
        EXPECT_STATEFUL(state, detail::index(static_cast<const Checked&&>(c), 0) == 8);
        EXPECT_STATEFUL(state, detail::index(static_cast<const Checked&&>(c), 1) == 9);
        EXPECT_STATEFUL(state, detail::index(static_cast<const Checked&&>(c), 2) == 10);
    };
    EXPECT_THAT(packet);
...
TEST("cpo index", "prefer checked indexing")
{
    ...
    static constexpr auto packet = [](auto& state) {
        Both b;
        EXPECT_STATEFUL(state, detail::index(b, 0) == 5);
        EXPECT_STATEFUL(state, detail::index(b, 1) == 6);
        EXPECT_STATEFUL(state, detail::index(b, 2) == 7);
        EXPECT_STATEFUL(state, detail::index(std::as_const(b), 0) == 6);
        EXPECT_STATEFUL(state, detail::index(std::as_const(b), 1) == 7);
        EXPECT_STATEFUL(state, detail::index(std::as_const(b), 2) == 8);
        EXPECT_STATEFUL(state, detail::index(Both{}, 0) == 7);
        EXPECT_STATEFUL(state, detail::index(Both{}, 1) == 8);
        EXPECT_STATEFUL(state, detail::index(Both{}, 2) == 9);
        EXPECT_STATEFUL(state, detail::index(static_cast<const Both&&>(b), 0) == 8);
        EXPECT_STATEFUL(state, detail::index(static_cast<const Both&&>(b), 1) == 9);
        EXPECT_STATEFUL(state, detail::index(static_cast<const Both&&>(b), 2) == 10);
    };
    EXPECT_THAT(packet);
