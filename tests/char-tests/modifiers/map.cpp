...
#define FN_MODIFIER_ASSERTER(P)                                                                    \
	[]<Parser PP>(PP) {                                                                            \
		DEP_ASSERT_MODIFIER_CALLABLE_R(map<sink_func>, (PP{}), (Map<PP, Const<sink_func>>{}),      \
			                           map<sink_func>, (P{}),  (Map<P, Const<sink_func>>{}));      \
		DEP_ASSERT_MODIFIER_MODULO_OPERABLE_R(PP{}, map<sink_func>, (Map<PP, Const<sink_func>>{}), \
			                                  P{},  map<sink_func>, (Map<P, Const<sink_func>>{})); \
	}(P{});
